...
import { UCSCTOPIC_TO_CLASSES } from "./UCSCTopic_To_Classes";
...
const namesSet = new Set(); // lowercased full/stripped titles
 const codesSet = new Set(); // normalized codes like "CSE 80A"
  const codesCanonSet = new Set(); // canonical codes like "CSE80A"
  ...
  (keys || Object.keys(TOPIC_MAP || {})).forEach((k) => {
    (TOPIC_MAP[k] || []).forEach((raw) => {
      ...
      // code variants
      for (const v of generateCodeVariants(full)) {
        codesSet.add(v);
      codesCanonSet.add(canonCode(v));
      }
      for (const v of generateCodeVariants(stripped)) {
        codesSet.add(v);
       codesCanonSet.add(canonCode(v));
      }
    });
 ...
 return { namesSet, codesSet, codesCanonSet };
}
function canonCode(s = "") {
  // Uppercase and remove spaces, hyphens, underscores
  return String(s).toUpperCase().replace(/[\s\-_]/g, "");
}

function matchesTopic(cls, namesSet, codesSet, codesCanonSet) {
  ...
  // generate variants from strings + any existing courseCode
  const variants = [
    ...generateCodeVariants(nFull),
    ...generateCodeVariants(nStripped),
    ...(cls.courseCode ? generateCodeVariants(cls.courseCode) : []),
  ];

  // also build variants from structured fields, if present
  const dept = cls.dept || cls.subject || cls.department || cls.subjectCode || null;
  const num  = cls.number || cls.courseNumber || cls.catalogNumber || cls.catalogNbr || null;
  if (dept && num) {
    const fromFields = `${dept} ${num}`;
    variants.push(...generateCodeVariants(fromFields));
  }

  // exact code matches
  if (variants.some(v => codesSet.has(v))) return true;

  // canonical (no-space/no-hyphen) matches
  if (variants.some(v => codesCanonSet.has(canonCode(v)))) return true;
...
useEffect(() => {
  // 1) Make sure the header always receives clicks
  const header = document.querySelector('.navy-header');
  if (header) {
    header.style.zIndex = '2147483647';     // max it out
    header.style.pointerEvents = 'auto';
  }

  // 2) If something is covering the top, disable its pointer events
  const topEl = document.elementFromPoint(window.innerWidth / 2, 20);
  if (topEl && header && !header.contains(topEl)) {
    // Turn off click-blocking on the culprit
    topEl.style.pointerEvents = 'none';

    // If it’s inside a big wrapper, also turn it off there
    if (topEl.parentElement && !header.contains(topEl.parentElement)) {
      topEl.parentElement.style.pointerEvents = 'none';
    }
  }
}, []);
  ...
  }, [selectedAreas, selectedGoals, dropdownValue, classesTaken, classDetails, geRequirements, refreshKey]);
 ...
 const handleRemoveClass = (className, area) => {
  ...
  const searchResults = (search.trim().length > 0 && Array.isArray(classDetails))
    ...
    : [];
// ====== UCSC requirements — DATA DRIVEN from geRequirements.json ======
...
const TOPIC_MAP = UCSCTOPIC_TO_CLASSES || {};
console.log("[ucsc] TOPIC_MAP typeof:", typeof UCSCTOPIC_TO_CLASSES,
            "keys:", UCSCTOPIC_TO_CLASSES ? Object.keys(UCSCTOPIC_TO_CLASSES).length : "undefined");
console.log("[ucsc] map has keys:", UCSCTOPIC_TO_CLASSES ? Object.keys(UCSCTOPIC_TO_CLASSES) : []);
console.log("[ucsc] selectedAreas:", selectedAreas);

// after you compute `pool` (right before filtering):
...
console.log("[ucsc] base size:", base.length);

// right after the topicKeys line:
const topicKeys = resolveTopicKeys(selectedAreas, TOPIC_MAP);
console.log("[ucsc] topicKeys:", topicKeys);

// if pool ends up empty, probe a couple classes:
base.slice(0, 8).forEach((c, i) => {
  const n = (c.className || "").split(" - ")[0];
  console.log("[ucsc-probe]", i, {
    className: c.className,
    courseCode: c.courseCode,
    dept: c.dept || c.subject || c.department || c.subjectCode,
    num: c.number || c.courseNumber || c.catalogNumber || c.catalogNbr
  });
});
    // Topic filter
  // Topic filter
...
if (hasTopics) {
  const topicKeys = resolveTopicKeys(selectedAreas, TOPIC_MAP);
const { namesSet, codesSet, codesCanonSet } = buildTopicSets(TOPIC_MAP, topicKeys);
 ...
 pool = base.filter((c) => matchesTopic(c, namesSet, codesSet, codesCanonSet));
...
// --- Fridays Off: prefer to fully exclude Friday classes, with graceful fallback
if (wantFridayOff) {
  const noFriday = pool.filter(c => !getClassDays(c).has("F"));

  // If we have enough non-Friday classes, go strict no-Friday
  if (noFriday.length >= numClasses) {
    pool = noFriday;
  } else if (noFriday.length > 0) {
    // Not enough: keep all no-Friday first, then allow Friday ones to fill
    const onlyFriday = pool.filter(c => getClassDays(c).has("F"));
    pool = noFriday.concat(onlyFriday);
  } // else: zero non-Friday options; keep pool as-is
}
...
const friPenaltyFor = (cls) =>
  wantFridayOff && getClassDays(cls).has("F") ? 2.5 : 0;

const scored = pool.map((c) => {
  const baseScore = wantEasy ? easeScore(c) : easeScore(c) * 0.5;

  // GE bonus: prefer classes that fill unmet UCSC areas
  const classAreas =
...
// Pass 1: only items that add a *new unmet* UCSC area
...
return picked.slice(0, numClasses).map((cls) => ({ ...cls }));
  ...
  return (
    <>
     <div style={{ paddingTop: '0px', position: 'relative', zIndex: 0 }}>
        {/* Title */}
        <div
          style={{
